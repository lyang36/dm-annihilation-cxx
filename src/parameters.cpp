...
void Parameters::setupUnits(){
    ...
    // This should not be setup from file, since it's setting up automatically from
    // rhoc * boxsize**3 (asumming the average density is at critical density)
    if(info.mass_to_Msun == -1){
        ...
        info.mass_to_Msun = codeunits.mass_to_Msun;
    }else{
        codeunits.mass_to_Msun = info.mass_to_Msun;
        codeunits.mass_to_cgs = codeunits.mass_to_Msun * units.Msun_in_cgs;
    }
...
void Parameters::setupHalo(){
    ...
    halo.Mvir_in_Msun = info.haloInfo.Mvir_in_Msun;
    halo.M200_in_Msun = info.haloInfo.M200_in_Msun;
    halo.M200crit_in_Msun = info.haloInfo.M200crit_in_Msun;
    halo.Rvir_in_Mpc = info.haloInfo.Rvir_in_Mpc;
    halo.R200_in_Mpc = info.haloInfo.R200_in_Mpc;
    halo.R200crit_in_Mpc = info.haloInfo.R200crit_in_Mpc;
    halo.Vmax_in_kms = info.haloInfo.Vmax_in_kms;
    halo.RVmax_in_kpc = info.haloInfo.RVmax_in_kpc;
    halo.rconverg_in_kpc = info.haloInfo.rconverg_in_kpc;
    
    for(int i = 0; i < 3; i++){
        halo.params_NFW[i] = info.haloInfo.params_NFW[i];
        halo.params_GNFW[i] = info.haloInfo.params_GNFW[i];
        halo.params_Einasto[i] = info.haloInfo.params_Einasto[i];
    }
 ...
 * horizontally.
 *
 * The rotating axis is vec(r) \cross vec(e1).
 * The rotating angle is costheta = vec(r) \dot vec(e1)
 * r=[-sin(theta)*cos(phi), -sin(theta)*sin(phi), -cos(theta)]
 * vec(r) \cross vec(e1) = [u, v, w]
 * c = cos(a) = vec(r) \dot vec(e1) = -sin(theta)*cos(phi)
 * Details could be searched from google: 
...
void Parameters::setupRotation(){
    ...
    // Rotating around x direction to make the align vector horizontal
...
Parameters::Parameters(string ifname){
    ...
    if (infofile.is_open()){
		while (infofile.good()){
			...
			if (temp == "Omega_M"){
            ...
            }else if (temp == "mass_to_Msun"){
                info.mass_to_Msun = atof(val);
				...
				do{
					...
					i++;
					val = strtok(NULL, " ");
				}while (val != NULL && i < MAX_NUM_MASS_RES);
            ...
            /**** Optional *****/
				...
				info.haloInfo.Mvir_in_Msun = atof(val);
			}else if (temp == "M200_in_Msun"){
                info.haloInfo.M200_in_Msun = atof(val);
			}else if (temp == "M200crit_in_Msun"){
                info.haloInfo.M200crit_in_Msun = atof(val);
			}else if (temp == "Rvir_in_Mpc"){
                info.haloInfo.Rvir_in_Mpc = atof(val);
			}else if (temp == "R200_in_Mpc"){
                info.haloInfo.R200_in_Mpc = atof(val);
			}else if (temp == "R200crit_in_Mpc"){
                info.haloInfo.R200crit_in_Mpc = atof(val);
			}else if (temp == "Vmax_in_kms"){
                info.haloInfo.Vmax_in_kms = atof(val);
			}else if (temp == "RVmax_in_kpc"){
                info.haloInfo.RVmax_in_kpc = atof(val);
			}else if (temp == "rconverg_in_kpc"){
                info.haloInfo.rconverg_in_kpc = atof(val);
			}else if (temp == "params_NFW"){
                info.haloInfo.params_NFW[0] = atof(val);
                val = strtok(NULL, " ");
                info.haloInfo.params_NFW[1] = atof(val);
                val = strtok(NULL, " ");
                info.haloInfo.params_NFW[2] = atof(val);
			}else if (temp == "params_GNFW"){
                info.haloInfo.params_GNFW[0] = atof(val);
                val = strtok(NULL, " ");
                info.haloInfo.params_GNFW[1] = atof(val);
                val = strtok(NULL, " ");
                info.haloInfo.params_GNFW[2] = atof(val);
			}else if (temp == "params_Einasto"){
                info.haloInfo.params_Einasto[0] = atof(val);
                val = strtok(NULL, " ");
                info.haloInfo.params_Einasto[1] = atof(val);
                val = strtok(NULL, " ");
                info.haloInfo.params_Einasto[2] = atof(val);
			}
            
            
            /******************Criticle information****************/
