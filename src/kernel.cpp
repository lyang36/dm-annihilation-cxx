...
__global__ void calculateNorm(int Npix, MAPTYPE * healpixX, MAPTYPE * healpixY, MAPTYPE * healpixZ, 
	...
	for(i = 0; i < numParts; i ++){
		...
		MAPTYPE d2 = acos(prod) / angular_radius;
		if(d2 > 2){
			continue;
		}
...
cudaError_t zeroLizeNorm(){
    //zerolize the norm
    cudaStatus = cudaMemcpy(norm_GPU, norm_CPU, memParts_ * sizeof(MAPTYPE), cudaMemmcpyHostToDevice);
...
cudaError_t initializeCUDA(MAPTYPE * healpixX, MAPTYPE * healpixY, MAPTYPE * healpixZ, int Npix, int memParts){
    ...
    cudaStatus = cudaMemcpy(healpixX_GPU, healpixX, Npix_ * sizeof(MAPTYPE), cudaMemmcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMemcpy failed -- copying HEALPIX X!\n");
        return cudaStatus;
    }
    ...
    cudaStatus = cudaMemcpy(healpixY_GPU, healpixY, Npix_ * sizeof(MAPTYPE), cudaMemmcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
    ...
    cudaStatus = cudaMemcpy(healpixZ_GPU, healpixZ, Npix_ * sizeof(MAPTYPE), cudaMemmcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
...
cudaError_t calulateMapWithCUDA(MAPTYPE * map, DMParticle * parts, int numParts){
    ...
    cudaStatus = cudaMemcpy(map_GPU, map, Npix_ * sizeof(MAPTYPE), cudaMemmcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMemcpy failed -- copying HEALPIX X!\n");
        return cudaStatus;
    }
    ...
    cudaStatus = cudaMemcpy(parts_GPU, parts, memParts_ * sizeof(DMParticle), cudaMemmcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMemcpy failed -- copying HEALPIX X!\n");
        return cudaStatus;
    }
    ...
    calculateNorm<<<gridsize, blocksize>>>(Npix_, healpixX_GPU, healpixY_GPU, healpixZ_GPU, 
    		numParts_GPU, parts_GPU, norm_GPU);
    ...
    calculateMap<<<gridsize, blocksize>>>(Npix_, healpixX_GPU, healpixY_GPU, healpixZ_GPU, 
    		numParts_GPU, parts_GPU, norm_GPU, map_GPU);
    cudaStatus = cudaThreadSynchronize();
    if( cudaStatus != cudaSuccess){
    ...
    //copy map back
     cudaStatus = cudaMemcpy(map, map_GPU, Npix_ * sizeof(MAPTYPE), cudaMemmcpyDeviceToHost);
     if (cudaStatus != cudaSuccess) {
